...
function createIdealWalk(px = 16, tkg) {
    ...
    function spriteClasses(sprite, env) {
        ...
        if (!terrainCache[key]) {
            const data = TERRAIN[sprite], out = new Uint8Array(PX * PX);
            if (data) {
    ...
    function setFloor(map) {
        ...
        for (let ty = 0; ty < mapHeight; ty++) {
            for (let tx = 0; tx < mapWidth; tx++) {
                ...
                masks.push(mask);
                const sc = spriteClasses(tkg.tileMap[mask.toString(16).toUpperCase().padStart(2, '0')], env);
                for (let y = 0; y < PX; y++) {
